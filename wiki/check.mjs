...
if (config.demo !== false) errors.push('Published Ashgrid documentation must disable demonstration mode.');
const pom = await readFile(path.join(root, '../pom.xml'), 'utf8');
const version = pom.match(/<version>([^<]+)<\/version>/)?.[1];
if (config.version !== version) errors.push(`Wiki version ${config.version} differs from pom.xml ${version}.`);
...
for (const id of ids) if (!navigationIds.has(id)) errors.push(`Page is absent from navigation: ${id}`);
...
for (const match of shell.matchAll(/href="#\/([^"?]+)"/g)) {
  if (!ids.has(match[1])) errors.push(`Shell links to unknown page: ${match[1]}`);
}
