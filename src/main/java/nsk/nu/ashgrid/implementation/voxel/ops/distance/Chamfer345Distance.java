...
public final class Chamfer345Distance implements DistanceTransform {
    ...
    private void V(int w, int h, int d, float[] out, int wh, int[][] nf, int[][] ne, int[][] nc, float wf, float we, float wc, int z, int y, int x) {
        int i = idx(w,wh,x,y,z);
        float v = out[i];

        for (int[] o : nf)   v = Math.min(v, neighbor(out, w,h,d, wh, x+o[0], y+o[1], z+o[2], wf));
        for (int[] o : ne)   v = Math.min(v, neighbor(out, w,h,d, wh, x+o[0], y+o[1], z+o[2], we));
        for (int[] o : nc)   v = Math.min(v, neighbor(out, w,h,d, wh, x+o[0], y+o[1], z+o[2], wc));

        out[i] = v;
    }

    private static int idx(int w,int wh,int x,int y,int z){ return z*wh + y*w + x; }

    private static float neighbor(float[] out, int w,int h,int d, int wh, int x,int y,int z, float wgt) {
        if (x<0||x>=w||y<0||y>=h||z<0||z>=d) return 1e9f;
        float nv = out[z*wh + y*w + x];
        return nv + wgt;
    }
}